...
def pass2_detail_filter(details: list[ListingDetail], cache: dict) -> list[ListingDetail]:
    ...
    for detail in details:
        ...
        for key, val in detail.raw_attributes.items():
            key_lower = key.lower()
            if not detail.price_chf and any(
                w in key_lower for w in ["price", "preis", "miete", "rent"]
            ):
                detail.price_chf = parse_price(val)
            if not detail.address and any(
                w in key_lower for w in ["address", "adresse", "location", "ort"]
            ):
                detail.address = val
            if not detail.num_people and any(
                w in key_lower for w in ["person", "roommate", "mitbewohner", "wg"]
            ):
