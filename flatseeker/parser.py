...
def parse_price(text: str) -> int | None:
    ...
    money_context = re.search(
        r"(?:CHF|Fr|Miete|rent|price|preis|kosten)[\s\S]{0,20}?(\d{3,4})", text, re.IGNORECASE
    )
    if money_context:
...
def is_sublet(text: str) -> bool:
    ...
    if "untermiete" in check:
        ...
        if re.search(
            r"untermiete.*(?:januar|februar|märz|april|mai|juni|juli|august|september|oktober|november|dezember).*(?:[-–&und]|bis).*(?:januar|februar|märz|april|mai|juni|juli|august|september|oktober|november|dezember)",
            check,
        ):
            return True
    ...
    if re.search(
        r"\d{1,2}\.?\s*(?:jan|feb|mär|mar|apr|mai|may|jun|jul|aug|sep|okt|oct|nov|dez|dec)\w*\s*[-–]\s*\d{1,2}\.?\s*(?:jan|feb|mär|mar|apr|mai|may|jun|jul|aug|sep|okt|oct|nov|dez|dec)",
        check,
    ):
        return True
...
def is_request_not_offer(text: str) -> bool:
    ...
    offer_patterns = [
        r"nachmieter.*gesucht",
        r"nachmieterin.*gesucht",
        r"mitbewohner.*gesucht",
        r"mitbewohnerin.*gesucht",
        r"suchen.*mitbewohner",
        r"suchen.*nachmieter",
        r"wir suchen.*für unsere",
        r"suchen.*für.*wg",
        r"zu vermieten",
        r"to rent",
        r"for rent",
